...
// Interface PUT https://edumatch.canadacentral.cloudapp.azure.com/admin/unban
describe("Unbanned user", () => {
    ...
    test('Should pass when admin and user are valid', async() => {
        ...
        expect(res.body).toEqual({ 
            message: "User with id " + mockUserId + " was unbanned successfully"
        })
    ...
    test('Should fail if not admin', async() => {
        ...
        expect(res.body).toEqual({ 
            message: "User is not admin and is not authorized to unban"
        })
        expect(mockAddedUsers[1].isBanned).toBeTruthy()
    })
